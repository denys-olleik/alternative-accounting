...
namespace Accounting.Service
{
  public class ClaimService : BaseService
  {
    ...
    public async Task<int> CreateRoleAsync(int userID, int organizationId, string role, string databaseName)
    {
      var factoryManager = new FactoryManager(_databaseName, _databasePassword);
      var claimManager = factoryManager.GetClaimManager();
      return await claimManager.CreateClaimAsync(userID, Claim.CustomClaimTypeConstants.Role, role, organizationId, userID, databaseName);
    }
    ...
    public async Task<Claim> GetAsync(int userID, int organizationId, string claimType, string claimValue)
    {
      var factoryManager = new FactoryManager(_databaseName, _databasePassword);
      var claimManager = factoryManager.GetClaimManager();
      return await claimManager.GetAsync(userID, organizationId, claimType, claimValue);
    }
